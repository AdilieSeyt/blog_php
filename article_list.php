...
if (isset($_GET['category_id'])) {
	...
	$queryArticles = $db->prepare('SELECT * FROM article 
	WHERE published_at <= NOW() AND is_published=1 AND category_id = ?
	...
	$queryArticles = $db->query('SELECT title, published_at, summary, article.id, name, article.image FROM article INNER JOIN category ON category_id = category.id WHERE is_published=1 ORDER BY published_at ASC');
}
